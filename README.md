<div align="center">
  ...
  <p><strong>A Next-Generation High-Performance 3D Game Engine built in pure Rust & WGPU, inspired by Unreal Engine 5.</strong></p>
...
**Oxyd Engine** is an open-source, ultra-fast, cross-platform 3D game engine engineered from the ground up in **Rust**. Designed with an interface and workflow inspired by **Unreal Engine 5**, Oxyd Engine provides developers, artists, and AI agents with modern tools to build, simulate, and render rich 3D worlds.
...
- ✈️ **Unreal Engine 5 Flight Navigation**: Real-time 3D viewport navigation using `Right Click + WASD / QE`, orbit look around, middle-click pan, smooth scroll zoom, and instant `Key F` focus.
...
- 📁 **Real-Time Content Drawer File Browser**: Dynamic filesystem browser matching UE5 styling (`Actors`, `Maps`, `Materials`, `Meshes`, `Textures`, `Decals`, `VFX`) with instant level loading.
...
- 📑 **Unreal-Style World Outliner & Details**: Double-click item to focus camera, right-click context menu (Duplicate, Toggle Visibility, Delete), and alternating row highlight.
